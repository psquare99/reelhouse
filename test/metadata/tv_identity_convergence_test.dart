...
void main() {
  ...
  group('Database mergeTvShows & mergeMovies', () {
    test('mergeTvShows correctly merges disjoint seasons without losing media sources', () async {
      ...
      // Perform Merge
      await db.mergeTvShows(sourceShowId: 'show-dup', targetShowId: 'show-main');
      ...
      expect(s4Sources.first.relativePath, 'Two and a Half Men 2003/Season 4/S04E01.mkv');
    ...
    test('mergeTvShows merges overlapping seasons and episodes, combining media sources and merging watch states', () async {
      ...
      // Perform Merge
      await db.mergeTvShows(sourceShowId: 'show-source', targetShowId: 'show-target');

      // Verify source show, season, and episode records are cleaned up
      expect(await db.findTvShowById('show-source'), isNull);
      expect(await (db.select(db.seasons)..where((s) => s.id.equals('source-s1'))).getSingleOrNull(), isNull);
      expect(await (db.select(db.episodes)..where((e) => e.id.equals('source-s1e1'))).getSingleOrNull(), isNull);
      ...
      expect(targetSources.map((s) => s.id).toList(), containsAll(['src-file-a', 'src-file-b']));
    ...
    test('mergeTvShows re-parents collections and deduplicates', () async {
      ...
      await db.mergeTvShows(sourceShowId: 'source-show', targetShowId: 'target-show');
    ...
    test('mergeMovies re-points sources and merges watch states', () async {
      ...
      await db.mergeMovies(sourceMovieId: 'movie-source', targetMovieId: 'movie-target');
      ...
      expect(sources.map((s) => s.id).toList(), containsAll(['src-inception-1', 'src-inception-2']));
    });
  });
  ...
  group('MetadataService Canonical Convergence', () {
    ...
    test('shows with different TMDB IDs do not merge', () async {
      ...
      await metadataService.applyTvShowMatch(
        'show-1',
        const TmdbTvSearchResult(id: 100, name: 'Show One', originalName: 'Show One'),
      );
      await metadataService.applyTvShowMatch(
        'show-2',
        const TmdbTvSearchResult(id: 200, name: 'Show Two', originalName: 'Show Two'),
      );
