...
void main() {
  ...
  setUp(() async {
    ...
    await db.into(db.storages).insert(
  ...
  group('Database Query Engine — Movie Queries', () {
    test('queryMovies returns all movies with availability projections and pagination', () async {
      ...
      await db.into(db.mediaSources).insert(
      ...
      expect(allResult.items[0].availability, equals(AvailabilityStatus.availableOnRemovableStorage));
      expect(allResult.items[1].displayTitle, equals('Inception'));
      expect(allResult.items[1].availability, equals(AvailabilityStatus.availableLocally));
      expect(allResult.items[2].displayTitle, equals('Interstellar'));
      expect(allResult.items[2].availability, equals(AvailabilityStatus.availableOnMultipleSources));
    });

    test('queryMovies pagination (limit, offset, hasMore, deterministic ordering)', () async {
      final now = DateTime(2026, 1, 1);
      for (var i = 1; i <= 5; i++) {
        await db.into(db.movies).insert(
          MoviesCompanion.insert(
    ...
    test('queryMovies Smart Views factories work through standard engine', () async {
      final now = DateTime(2026, 1, 1);
      ...
      final continueWatching = await db.queryMovies(MovieQuery.continueWatching());
    ...
    test('queryMovies filtering by collection membership', () async {
      ...
      final result = await db.queryMovies(MovieQuery.forCollection('col-marvel'));
    ...
    test('queryMovies search (title mode and all mode)', () async {
      ...
      await db.into(db.movies).insert(
        MoviesCompanion.insert(
          ...
          overview: const drift.Value('A blade runner must pursue replicants.'),
          createdAt: now,
          updatedAt: now,
        ),
      );
  ...
  group('Database Query Engine — TV Show Queries', () {
    ...
    test('queryTvShows filtering by premiere year range', () async {
      ...
      final result = await db.queryTvShows(
        const TvShowQuery(
          filter: TvShowFilter(
            yearRange: YearRange.since(2020),
          ),
        ),
      );
      expect(result.totalCount, equals(1));
  ...
  group('Database Query Engine — Season & Episode Queries', () {
    test('querySeasons and queryEpisodes return ordered items for a show', () async {
      final now = DateTime(2026, 1, 1);
      ...
      // Query seasons
      final seasonsResult = await db.querySeasons(SeasonQuery.forShow('tv-show-1'));
      ...
      final episodesResult = await db.queryEpisodes(EpisodeQuery.forSeason('s-1'));
      ...
      expect(episodesResult.items[1].watchState, equals(WatchState.inProgress));
      expect(episodesResult.items[1].playbackPositionSeconds, equals(450));

      // Query continue watching episodes
      final contEpisodes = await db.queryEpisodes(EpisodeQuery.continueWatching());
  ...
  group('Database Query Engine — Collection Queries', () {
    test('queryCollections returns sorted collections and supports search', () async {
      final now = DateTime(2026, 1, 1);
      ...
      // Search collections
      final search = await db.queryCollections(CollectionQuery.search('Universe'));
