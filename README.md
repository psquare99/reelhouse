...
- **M4.1 — Cinema Polish, Offline Library & Playback Preparation**: Refined obsidian/charcoal/amber visual identity with premium elevation and border system; breathing room across poster grids (`maxCrossAxisExtent: 260`, spacing 28/32); Offline Library screen with All/Movies/TV filter tabs; Schema v3 metadata provenance columns (`metadataProvider`, `providerItemId`, `metadataUpdatedAt`); `PlaybackLauncherService` with full Windows VLC + system-default dispatch, storage connectivity verification, and failure diagnostics; honest M5-preview Download dialog replacing fake in-progress states; `FallbackMetadataOrchestrator` with OMDb (movie) and TVmaze (TV) fallbacks, 85% confidence gate, and provider provenance tracking.
